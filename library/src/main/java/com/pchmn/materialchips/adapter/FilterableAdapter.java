        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint.length() == 0) {
                ...
                for (ChipInterface chip : originalList) {
                    if (chip.getLabel().toString().toLowerCase().contains(filterPattern)) {
                        filteredList.add(chip);
                    }
                    else if(chip.getInfo() != null && chip.getInfo().toString().toLowerCase().replaceAll("\\s", "").contains(filterPattern)) {
                        filteredList.add(chip);
                    }
                }
            }
